...
const App = () => {
  ...
  const [readingSettings, setReadingSettings] = useState(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : { fontSize: 18, lineHeight: 1.8 };
    } catch {
      return { fontSize: 18, lineHeight: 1.8 };
    }
  });
          ...
          <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4">
            <div className={`rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden ${darkMode ? 'bg-gray-800 text-gray-100' : 'bg-white'}`}>
              <div className={`p-6 text-center ${darkMode ? 'bg-gray-700' : 'bg-gradient-to-br from-orange-500 to-amber-500'} text-white`}>
                ...
                <p className="text-sm opacity-90 mt-1">Applies to all bhajans</p>
              </div>
              <div className="p-6 space-y-4">
                {/* Font Size */}
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`w-10 h-10 rounded-lg font-bold text-lg ${darkMode ? 'bg-gray-700 text-gray-200 hover:bg-gray-600' : 'bg-orange-100 text-amber-800 hover:bg-orange-200'}`}
                    >
                      A−
                    </button>
                    <span className="text-sm font-mono w-10 text-center">{readingSettings.fontSize}px</span>
                    <button
                      onClick={() => setReadingSettings(prev => ({ ...prev, fontSize: Math.min(32, prev.fontSize + 2) }))}
                      className={`w-10 h-10 rounded-lg font-bold text-lg ${darkMode ? 'bg-gray-700 text-gray-200 hover:bg-gray-600' : 'bg-orange-100 text-amber-800 hover:bg-orange-200'}`}
                    >
                      A+
                    </button>
                ...
                {/* Line Spacing */}
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold">Reading View</span>
                  <button
                    onClick={() => setReadingSettings(prev => ({ ...prev, lineHeight: prev.lineHeight === 1.8 ? 2.2 : 1.8 }))}
                    className={`px-4 py-2 rounded-lg text-sm font-semibold ${
                      readingSettings.lineHeight >= 2.0
                        ? 'bg-orange-500 text-white shadow-md'
                        : darkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-orange-100 text-amber-800 hover:bg-orange-200'
                    }`}
                  >
                    {readingSettings.lineHeight >= 2.0 ? '✓ Wide' : 'Default'}
                  </button>
                </div>

                {/* Fit to Screen */}
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold">Fit to Screen</span>
                  <button
                    onClick={() => setReadingSettings(prev => ({ ...prev, fontSize: 18, lineHeight: 1.8 }))}
                    className={`px-4 py-2 rounded-lg text-sm font-semibold ${darkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-orange-100 text-amber-800 hover:bg-orange-200'}`}
                  >
                    Reset
                  </button>
                </div>
                ...
                {/* Keep Screen On */}
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold">🔦 Keep Screen On</span>
                  <button
                    onClick={async () => {
                      try {
                        if ('wakeLock' in navigator) {
                          const wakeLock = await navigator.wakeLock.request('screen');
                          setLiveWakeLock(wakeLock);
                          alert('Screen will stay awake while reading');
                        } else {
                          alert('Screen wake lock not supported on this device');
                        }
                      } catch (err) {
                        alert('Could not keep screen on: ' + err.message);
                      }
                    }}
                    className={`px-4 py-2 rounded-lg text-sm font-semibold ${
                      liveWakeLock
                  ...
                  >
                    {liveWakeLock ? '✓ Active' : 'Enable'}
                  </button>
                </div>

                <button
                  onClick={() => setShowReadingSettings(false)}
                  className="w-full bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold py-3 rounded-xl shadow-lg mt-2"
                >
                  Done
                </button>
              </div>
          ...
          {currentView === 'bhajan-detail' && selectedBhajan && (
            <>
              ...
              <div className={`rounded-2xl shadow-lg p-6 md:p-8 mb-4 ${darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white'}`}>
                <h1 className={`text-2xl md:text-3xl font-bold mb-3 truncate ${darkMode ? 'text-amber-100' : 'text-amber-900'}`}>
                  {selectedBhajan.title}
                </h1>

                {selectedBhajan.dhun && (
                  <div className={`border-l-4 border-orange-400 p-3 rounded-r-lg mb-3 ${darkMode ? 'bg-gray-700' : 'bg-orange-50'}`}>
                ...
                <div className="flex flex-wrap items-center gap-2 mb-4">
                  ...
                  <button
                    ...
                    className={`ml-auto px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-1 transition-colors ${darkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-orange-100 text-amber-800 hover:bg-orange-200'}`}
...
export default AppWithErrorBoundary;import React, { useState, useEffect, useCallback, useRef, useMemo } from 'react';
